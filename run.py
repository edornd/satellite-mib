...
from dataset import VOCSegmentationIncremental, AdeSegmentationIncremental, PotsdamIncremental, VaihingenIncremental
...
def get_augmentations(opts: Namespace):
    if opts.dataset in ("vaihingen", "potsdam"):
        ...
        train_transform = alb.Compose([
            alb.Flip(),
            alb.ShiftScaleRotate(shift_limit=0.2,
                                 scale_limit=(-0.2, 0.5),
                                 rotate_limit=90,
                                 border_mode=4,
                                 interpolation=1),
            alb.Normalize(mean=means, std=stds),    # should be 0-255 uint8
            ToTensorV2()
        ])
...
def main(opts):
    ...
    if rank == 0 and opts.sample_num > 0:
        sample_ids = np.random.choice(len(val_loader), opts.sample_num,
                                      replace=False)    # sample idxs for visualization
        logger.info(f"The samples id are {sample_ids}")
    else:
        sample_ids = None

    label2color = utils.Label2Color(cmap=utils.color_map(opts.dataset))    # convert labels to images
    denorm = utils.Denormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224,
                                                                0.225])    # de-normalization for original images
