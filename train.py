...
from utils.loss import (BCEWithLogitsLossWithIgnoreIndex, FocalLoss, IcarlLoss, KnowledgeDistillationLoss,
                        UnbiasedCrossEntropy, UnbiasedFocalLoss, UnbiasedKnowledgeDistillationLoss)
...
class Trainer:
    ...
    def train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=5, logger=None):
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if (cur_step + 1) % print_int == 0:
                ...
                logger.debug(f"\nEpoch {cur_epoch}, Batch {cur_step + 1}/{len(train_loader)}," f" Loss={interval_loss}")
